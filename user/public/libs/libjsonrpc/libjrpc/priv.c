...
int (*jrpc_recv_callback)(char *buf, int len);
int (*jrpc_send_callback)(char *buf, int len);
#endif
...
#if defined(__SC_BUILD__)
        if(jrpc_send_callback) {
            jrpc_send_callback(buf, buflen);
        }
#endif
...
#if defined(__SC_BUILD__)
        if(jrpc_recv_callback) {
            jrpc_recv_callback(buf, rb);
        }
#endif
