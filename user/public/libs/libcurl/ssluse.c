 ...
 *
 * $Id: ssluse.c,v 1.1.1.1 2008-04-07 08:44:26 dody_zhang Exp $
 ***************************************************************************/
...
/* The following is done because of a bug in 0.9.6b */
    ...
    if (!peer_CN) {
      if(data->set.ssl.verifyhost > 1) {
		...
		if(data->set.ssl.verifyhost > 1) {
#ifdef __SC_BUILD__	
			if(data->set.host_name != NULL)
			{
				if(!cert_hostcheck((const char *)peer_CN, data->set.host_name))
				{
					failf(data, "SSL: certificate subject name '%s' does not match "
						  "target host name '%s'", peer_CN, data->set.host_name);
