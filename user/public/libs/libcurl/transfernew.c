 ...
 *
 * $Id: transfernew.c,v 1.1.1.1 2008-04-07 08:44:26 dody_zhang Exp $
 ***************************************************************************/
...
cprintf("laoyou%d,readrc=%d,nread=%d\n",__LINE__,readrc,nread);
...
cprintf("laoyou%d,contentlength=%d,nread=%d\n",__LINE__,contentlength,nread);
//              if(contentlength >= 0)
              if(contentlength > 0)
                conn->size = contentlength;
              else if(!contentlength)
                conn->size = (((nread - 26)>0)?(nread - 26):0);//laoyou for CISCO download
              else {
             ...
             */
cprintf("laoyou%d,received nread=%d,conn->size=%d,reallength=%d\n",__LINE__,nread,conn->size,strlen(k->str));
...
cprintf("laoyou%d,k->hbuflen=%d,result=%d\n",__LINE__,k->hbuflen,result);
            if(result)
              return result;
...
cprintf("laoyou%d,stop_reading=%d,conn->size=%d,conn->maxdownload=%d\n",__LINE__,stop_reading,conn->maxdownload);          
          if(stop_reading)
