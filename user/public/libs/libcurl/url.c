 ...
 *
 * $Id: url.c,v 1.1.1.1 2008-04-07 08:44:27 dody_zhang Exp $
 ***************************************************************************/
...
#endif
#ifdef __SC_BUILD__
    data->set.host_name = NULL;
#endif
    data->set.brokentimeout=0;
  }
...
CURLcode Curl_setopt(struct SessionHandle *data, CURLoption option, ...)
{
  ...
  case CURLOPT_UNIQUECOOKIESESSION:
    data->set.unique_cookie_session = (bool)va_arg(param, long);
    break;
#endif
#endif
  ...
  case CURLOPT_LOCALPORT:
    data->set.localport = va_arg(param, long);
    break;

  case CURLOPT_LOCALPORTRANGE:
    data->set.localportrange = va_arg(param, long);
    break;
  ...
  case CURLOPT_SOURCE_POSTQUOTE:
	...
	case CURLOPT_DSCP:
		data->set.dscp = va_arg(param, int);
		break;

	case CURLOPT_PRIORITY:
		data->set.priority = va_arg(param, int);
		break;

  case CURLOPT_BROKENTIMEOUT:
    /*
     * The maximum time you allow curl to use for a single transfer not recevie any resopnse.
     * operation.
     */
    data->set.brokentimeout = va_arg(param, long);
    break;
#ifdef __SC_BUILD__
  case CURLOPT_HOST_NAME:
    /*
     * fill host name 
     */
    data->set.host_name = va_arg(param, char *);
    break;
#endif
  default:
                              ...
                              bool *connected)
{
  ...
  infof(conn->data, "About to connect() to %s port %d\n",
        (conn->data->change.proxy?conn->proxy.name:conn->host.name), conn->port);
...
#define LEAST_PATH_ALLOC 256
   ...
   * name resolve.
   *************************************************************/
#if 0//Leon disable the alarm fucntion for memory leak and open too many sockets fd issues cause by libcurl.
