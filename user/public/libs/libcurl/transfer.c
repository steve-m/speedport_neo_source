 ...
 *                             \___|\___/|_| \_\_____|
 *
 * Copyright (C) 1998, 2005 - 2004, Daniel Stenberg, <daniel@haxx.se>, et al.
 *
 ...
 *
 * $Id: transfer.c,v 1.1.1.1 2008-04-07 08:44:27 dody_zhang Exp $
 ***************************************************************************/
...
//cprintf("**laoyou**%s,%d,httpcode=%d,httpreq=%s\n",__FILE__,__LINE__,
//            k->httpcode,(data->set.httpreq==HTTPREQ_PUT)?"HTTPREQ_PUT":"other");
              if ((data->set.httpreq==HTTPREQ_PUT) && (405 == k->httpcode)) {
                /* 405 Method Not Allowed, server can not allowed to write 
                 * add for Upload RPC method, laoyou@2007,Jul
                 */
            ...
            if ((k->httpcode != 416) &&
              ...
              if(contentlength > 0)
                conn->size = contentlength;
                  ...
                  conn->size = (((nread - 26)>766)?(nread - 26):0);//laoyou for CISCO download
              else {
              ...
              if(data->set.http_follow_location) {
                ...
                if(ptr != start) {
                  ...
                  *ptr = backup; /* restore ending letter */
                  if(!conn->newurl)
                    return CURLE_OUT_OF_MEMORY;
#ifdef SUPPORT_OD69_CERT
                  else if((k->httpcode==307 || k->httpcode==302) && data->set.unique_cookie_session)
          ...
          if(!conn->bits.chunk && (nread || k->badheader || is_empty_data)) {
            ...
            if(k->badheader < HEADER_ALLBAD) {
              ...
              case IDENTITY:
                ...
                if(!k->ignorebody){
#ifdef CWMP_DEBUG
printf("%s,%s,%d nread=%ld\n",__FIEL__,__FUNCTION__,__LINE__,nread);
#endif
			/*=====================================
			* Add by Jeff Sun @ Sep,29,2005
			* Fix Can't do SetParameterValues method when
			* ACS send an incorrect Content-Length Http packet.
			*/
			if(nread<strlen(k->str)) nread=strlen(k->str);

			/*Contert-Length in Cisco ACS's InformResponse packets is 
			* incorrect. So,if we want to use this ACS, we can only do is
			* like the following line.
			if(nread==536 && strlen(k->str)==535) nread=535;
			*/
			/*=====================================*/
#ifdef CWMP_DEBUG
printf("%s,%s,%d nread=%ld\n",__FIEL__,__FUNCTION__,__LINE__,nread);
  ...
  if( data->set.brokentimeout && didwhat==0)
  { //coda add for internet broken(or download broken)
    /* no read no write, this is a timeout*/
    if((Curl_tvdiff(k->now, k->start)/1000) >= data->set.brokentimeout ){
        failf(data, "Operation timed out with %" FORMAT_OFF_T
            " out of %" FORMAT_OFF_T " bytes received",k->bytecount, conn->size);
        return CURLE_OPERATION_TIMEOUTED;
    }
  }
                                      ...
                                      here */
{
  ...
  case 302: /* Found */
    ...
    */
#ifdef SUPPORT_OD69
	/*od69 case 5.30 HTTP Redirection Test - 302 Redirect*/
  	break;   /* (treat 302 like 307) POST request not GET request*/
#endif
  case 303: /* See Other */
