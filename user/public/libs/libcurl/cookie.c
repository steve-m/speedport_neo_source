 ...
 *
 * $Id: cookie.c,v 1.1.1.1 2008-04-07 08:44:27 dody_zhang Exp $
 ***************************************************************************/
...
Curl_cookie_add(struct SessionHandle *data,
                ...
                char *path,      /* full path used when this cookie is set,
                                    ...
                                    unless set */
                int type)       /* indicate that is set-cookie or set-cookie2 */
{
  struct Cookie *clist;
  char *what;
  char name[MAX_NAME];
        ...
        if(1 <= sscanf(ptr, "%" MAX_NAME_TXT "[^;=]=%"
          ...
          if(strequal("path", name)) {
            ...
            co->path=strdup(whatptr);
          	/**** remove end '"' *****/
          	pathlen=strlen(whatptr);
          	if(co->path[pathlen-1]=='\"') co->path[pathlen-1]=0;
  ...
  replace_old = FALSE;
  while(clist) {
    if((type==1)&&strequal(clist->name, co->name)) {
      /* for set-cookie, the names are identical 
       * but for set-cookie2, An origin server MAY include multiple Set-Cookie2
       * headers with the same name in a response
       * add by laoyou @ 2007,Jun.
       */
  ...
  if(fp) {
    ...
    if(line) {
      while(fgets(line, MAX_COOKIE_LINE, fp)) {
        ...
        else if(checkprefix("Set-Cookie2:", line)){
        	/* laoyou added on May.4,2006
        	 * support Set-Cookie2 header
        	 */
        	lineptr=&line[12];
        	headerline=TRUE;
        }
        ...
        Curl_cookie_add(data, c, headerline, lineptr, NULL, NULL, 1);
      }
