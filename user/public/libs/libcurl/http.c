 ...
 *
 * $Id: http.c,v 1.1.1.1 2008-04-07 08:44:26 dody_zhang Exp $
 ***************************************************************************/
                        ...
                        CURLcode status)
{
  ...
  if(http->send_buffer) {
    ...
    /*martin_huang: we should check this before free, anyway this is right here*/
    if(buff->buffer)
    	free(buff->buffer);
    free(buff);
...
//                              "Content-Type: application/x-www-form-urlencoded\r\n");
                            "Content-Type: text/xml; charset=\"utf-8\"\r\n");//Modified by Jeff Sun@Sep.09.2005
        if(result)
          return result;
      }
	
#if 1//Add by Jeff Sun@Sep.09.2005
	//modify by laoyou @Feb.27.2006
	if(data->set.postfieldsize){  
		/*when there is more than one envelope in a single HTTP Request,
		 *when there is a SOAP response in an HTTP Request, 
	     *or when there is a SOAP fault resposne,
	     *the SOAPAction header in the HTTP Request MUST have no value
    ...
    add_bufferf(req_buffer,"");
#else
    add_bufferf(req_buffer,"Expect: 100-continue\r\n");
#endif
            data->set.expect100header = TRUE;
          }
#if 1
	else{
        add_bufferf(req_buffer,"Connection: keep-alive\r\n");
	}
#endif
