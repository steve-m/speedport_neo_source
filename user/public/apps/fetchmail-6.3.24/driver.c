	...
	for (idp = ctl->mailboxes; idp; idp = idp->next)
	{
		...
		if(seen_only){
			if(ctl->server.protocol==P_IMAP){
				gen_send(mailserver_socket, "UID STORE %s +FLAGS (\\Seen)", mail_uid);
				for(recv_count = 0; recv_count < 2; recv_count++){
					gen_recv(mailserver_socket, buf, sizeof(buf));
					printf("seen buf = %s\n", buf);
					if(strncasecmp(strstr(buf, " "), " OK", 3) == 0){
						printf("OK\n");		
						break;
					}else if(strncasecmp(strstr(buf, " "), " BAD", 4) == 0){
						printf("BAD\n");
						break;
					}else if(strncasecmp(strstr(buf, " "), " NO", 3) == 0){
						printf("NO\n");
						break;
					}
				}
				...
				report(stdout,"protocol doesn't support  set seen flag");
		}
		if(delete_only){
			if(ctl->server.protocol==P_IMAP){
				gen_send(mailserver_socket, "UID STORE %s +FLAGS (\\Deleted)", mail_uid);
				for(recv_count=0;recv_count<5;recv_count++){
					gen_recv(mailserver_socket, buf, sizeof(buf));
					printf("dele buf = %s\n", buf);
					if(strncasecmp(strstr(buf, " ")," OK",3) == 0){
							printf("OK\n");
				...
				gen_send(mailserver_socket, "EXPUNGE");
				for(recv_count=0;recv_count<3;recv_count++){
					gen_recv(mailserver_socket, buf, sizeof(buf));
					printf("dele buf = %s\n", buf);
					if(strstr(buf,"OK"))
							break;
				...
				//gen_send(mailserver_socket,"DELE %s",mail_uid);
				//gen_recv(mailserver_socket, buf, sizeof(buf));
				//if(strncmp(buf, "-ERR", 4) == 0){
				//	printf("delete error\n");
					gen_send(mailserver_socket, "UIDL");
					for(recv_count=0;recv_count<500;recv_count++){
						gen_recv(mailserver_socket, buf, sizeof(buf));
						printf("buf =%s\n", buf);
						if(buf[0]=='.'){
							break;
						}
						if(strstr(buf,mail_uid)){
							p=strchr(buf,' ');
							if(p){
								*p=0;
								strcpy(number,buf);
					...
					if(find_num){
						printf("number = %s\n", number);
						gen_send(mailserver_socket, "DELE %s", number);
						gen_recv(mailserver_socket, buf, sizeof(buf));
						printf("dele buf = %s\n", buf);
					}
				//}
		...
		else if (count > 0 &&!seen_only&&!delete_only)
		{    
		    int		i;
