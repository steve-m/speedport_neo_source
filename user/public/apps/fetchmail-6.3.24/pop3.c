...
static void set_peek_capable(struct query *ctl)
{
    ...
    if(ctl->keep) peek_capable=1;  //SC_BUILD for gmail keep mails
}
...
int pop3_fetch_uid(int sock, struct query *ctl, int number, int *lenp)
{
	char *ptr;
    char buf[POPBUFSIZE+1];
	FILE *fp=NULL;
...
/* request nth message */
{
    int ok;
    char buf[POPBUFSIZE+1];
	pop3_fetch_uid(sock,ctl,number,lenp);
#ifdef SDPS_ENABLE
    /*
