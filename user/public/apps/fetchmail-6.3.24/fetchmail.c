...
flag seen_only=FALSE;	    /* if --seen was set */
flag delete_only=FALSE;	    /* if --delete was set */
flag versioninfo;	    /* emit only version info */
char *mail_uid;		    /* mail uid for delete */
...
/* to be executed at the end of a poll cycle */
{
     ...
     */
    //if (!check_only) //don't write id file on readonly file system of VD168
