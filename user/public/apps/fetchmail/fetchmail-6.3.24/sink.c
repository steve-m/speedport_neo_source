...
/* ship a line to the given control block's output sink (SMTP server or MDA) */
{
	...
	*tp = '\0';
	...
	if(plain_flag==0){
		if(pMydata=strstr(buf,"From: ")){
			//pReturn=strstr(pMydata,"\x0d\x0a");
			pReturn=strchr(pMydata,'\n');
			if(pReturn) *pReturn=0;
			UnMimeHeader(pMydata);
			fputs(pMydata,sinkfp);	
			*pReturn='\n';
			fputs("\n",sinkfp);	
		}
		if(pMydata=strstr(buf,"Subject: ")){
			pReturn=strchr(pMydata,'\n');
			if(pReturn) *pReturn=0;
			UnMimeHeader(pMydata);
			fputs(pMydata,sinkfp);	
		...
		if(pMydata=strstr(buf,"text/plain")){
			pReturn=strstr(pMydata,"charset=");
			if(!pReturn) {
				pReturn=strchr(pMydata,'\n');
				if(pReturn)
					*pReturn=0;
	...
	}else if(plain_flag==1){
		if(strstr(buf,boundary_buf)){
			n=last-buf;
			plain_flag=2;
		}else{
...
static int open_bsmtp_sink(struct query *ctl, struct msgblk *msg,
	      int *good_addresses, int *bad_addresses,int num)
/* open a BSMTP stream */
{
    struct	idlist *idp;
    int		need_anglebrs;
	...
	plain_flag=0;
	text_length=0;
	fprintf(sinkfp,"MAIL NUM:%d\n",num);
	fprintf(sinkfp,"MAIL STATUS:%d\n",is_seen(num));
...
/* set up sinkfp to be an input sink we can ship a message to */
{
    *bad_addresses = *good_addresses = 0;
	...
	return(open_bsmtp_sink(ctl, msg, good_addresses, bad_addresses,num));
    /* 
...
/* set up output sink for a mailed warning to calling user */
{
	...
	status = open_sink(ctl, &reply, &good, &bad,0);
