...
int is_seen(int num)
{
	int i;
	for(i=0;i<unseen;i++)
		if(unseen_messages[i]==num) return 0;
	return 1;
}
...
#endif
static int imap_fetch_uid(int sock, struct query *ctl,int number,int *lenp)
{
    char buf [MSGBUFSIZE+1];
	char uid [128]={0};
    char *p1,*p2;
	FILE *fp=NULL;
    gen_send(sock, "FETCH %d UID", number);
	gen_recv(sock, buf, sizeof(buf));
	p1=strstr(buf,"UID");
	if(p1){
		p2=strchr(p1,')');
		*p2=0;
	...
	fp=fopen(ctl->bsmtp,"a+");
	if(fp){
		fprintf(fp,"MAIL UID:%s\n",uid);
		fclose(fp);
	}
   	return 0;
}
...
/* request headers of nth message */
{
	...
	imap_fetch_uid(sock,ctl,number,lenp);
    /* expunges change the fetch numbers */
    number -= expunged;
