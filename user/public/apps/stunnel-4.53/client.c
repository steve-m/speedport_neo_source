...
static void lsdcc_https_security(int pass)
{
	FILE *fp=NULL;
	
	if(pass)
	{
		fp = fopen(LSDCC_SECURITY_FILE, "w");
		if(fp)
			fclose(fp);
		else
			s_log(LOG_DEBUG, "%s open %s failed", __FUNCTION__, LSDCC_SECURITY_FILE);
	}
	else
		unlink(LSDCC_SECURITY_FILE);
}
...
#if 0//Sercomm marc ++: No libstunnel.so, I build env.o into stunnel.
            if(c->opt->option.transparent_src) {
