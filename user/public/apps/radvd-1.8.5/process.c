...
static void process_ra(struct Interface *, unsigned char *msg, int len,
	struct sockaddr_in6 *);
static void process_route_info(char *ifname, unsigned char *msg, int len,
	struct sockaddr_in6 *);
	...
	struct sockaddr_in6 *addr, struct in6_pktinfo *pkt_info, int hoplimit)
{
	...
	if (icmph->icmp6_type == ND_ROUTER_ADVERT)
	{
		...
		if_indextoname(pkt_info->ipi6_ifindex, ifname);
		if (strcmp(ifname, "ppp1") == 0) {
			process_route_info(ifname, msg, len, addr);
			return;
		}
	}
	...
	struct sockaddr_in6 *addr)
{
	...
	if (iface->UnicastOnly) {
		...
		next = iface->MinDelayBetweenRAs - (ts.tv_sec + ts.tv_nsec / 1000000000.0) + (iface->last_multicast.tv_sec + iface->last_multicast.tv_nsec / 1000000000.0) + delay/1000.0;
		...
		if (deprecate_flag == 1) {
			sysinfo(&info);
			/* calculate how many seconds till the next supposed periodic RA */
			interval_cnt = (info.uptime - radvd_deprecate_start_time) / iface->MaxRtrAdvInterval;
			next = radvd_deprecate_start_time + iface->MaxRtrAdvInterval * (interval_cnt + 1) - info.uptime;
		} else {
...
static void
process_route_info(char* ifname, unsigned char *msg, int len,
	struct sockaddr_in6 *addr)
{
	struct nd_router_advert *radvert;
	char addr_str[INET6_ADDRSTRLEN];
	...
	if (strcmp(ifname, "ppp1") == 0) {
		if (strcmp(sal_ipv6_get_ra_routeinfo(), "1") == 0) {
			time(&current_t);
			routeinfo_enable = 1;
		}
	}
        ...
        struct sc_routeinfo_option *sc_routeinfo_opts = NULL;
        evt_len = sizeof(struct sc_event_msg) + sizeof(struct sc_routeinfo_option);
        sc_evt_msg = malloc(evt_len);
        sc_routeinfo_opts = sc_evt_msg->buf; 
	...
	while (len > 0)
	{
		...
		{
			case ND_OPT_ROUTE_INFORMATION:
				/* check route info in WAN's RA */
				if (routeinfo_enable == 1) {
                                    rinfo = (struct nd_opt_route_info_local *) opt_str;
		                    print_addr(&rinfo->nd_opt_ri_prefix, prefix_str);

			            strcpy(sc_routeinfo_opts->sourcerouter, addr_str);
                                    strcpy(sc_routeinfo_opts->prefix, prefix_str);
                   
		        switch (((rinfo->nd_opt_ri_flags_reserved & 0x18) >> 3) & 0xff) {
	        	        case 0:
                                    strcpy(sc_routeinfo_opts->preferred_routeflag, "Medium");       
			            break;
			        case 1:
			        strcpy(sc_routeinfo_opts->preferred_routeflag, "Heigh");
                                    break;
			        case 2:
			            /* reserved, ignore */
			            break;
			        case 3:
			            strcpy(sc_routeinfo_opts->preferred_routeflag, "Low");
                                    break;
			        default:
			            strcpy(sc_routeinfo_opts->preferred_routeflag, "Medium");
                                    break;
                 ...
                 /* XXX: we check DFLT_AdvRouteLifetime by interface, and it's outside of context here */
    		        routelftime = ntohl(rinfo->nd_opt_ri_lifetime);
                        sc_routeinfo_opts->routelftime = routelftime;
                        sc_routeinfo_opts->current_t = (unsigned int)current_t;
