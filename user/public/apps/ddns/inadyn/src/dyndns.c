...
static char* getlastline(char *dest)
{
    char    *p = dest;
    char    *q = dest;
...
static RC_TYPE do_update_alias_table(DYN_DNS_CLIENT *p_self)
{
    ...
    char	tbuf[DYNDNS_HTTP_RESPONSE_BUFFER_SIZE+1] = {0};
	RC_TYPE rc = RC_OK;
				...
				if (rc == RC_OK)
				{
					...
					if (update_ok)
					{
						...
						DBG_PRINTF((LOG_WARNING,"I:" MODULE_TAG "Alias '%s' to IP '%s' updated successful.\n", 
							p_self->alias_info.names[i].name,
							p_self->info.my_ip_address.name));                        
						sprintf(syslog_str, "Alias '%s' to IP '%s' updated successful.\n", 
							p_self->alias_info.names[i].name,
							p_self->info.my_ip_address.name);                        
					...
					{							
                        ...
                        strcpy(tbuf, http_tr.p_rsp);
                        syslog(LOG_INFO, "%s"LOG_ERRCODE_D002, log_type_db[GENERAL_LOG], getlastline(tbuf));
