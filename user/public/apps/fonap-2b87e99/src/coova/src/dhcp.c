...
#ifdef SUPPORT_FON
static int char_to_unchar(const char* str, unsigned char* digit16)       
{       
...
static int fon_dhcp_cleanup(struct dhcp_t *this)
{
    bstring s = 0;
    char mac_buf[20] = {0};
    uint8_t mac[20];
    ...
    /* add free mac to file */
    unlink("/tmp/fon_dhcp_list");
    dhcp_list(dhcp, s, 0, 0, LIST_SHORT_FMT);
    system("/etc/fon/fon_dhcp_handle");

    if (access("/tmp/fon_dhcp_list", F_OK) != 0)
    {
        log_err(0, "reached max connections!");
        return -1;
    }
    if ((fp = fopen("/tmp/fon_dhcp_list", "r")) == NULL)
    {
        log_err(0, "opem /tmp/fon_dhcp_list failed!");
        return -1;
    }
    /* release free mac */
    while(1)
    {
        fgets(mac_buf,18,fp);
        if(feof(fp)) break;
        char_to_unchar(mac_buf,mac);
        dhcp_release_mac(dhcp, mac, RADIUS_TERMINATE_CAUSE_ADMIN_RESET);
        --connections;
    }
    fclose(fp);
    unlink("/tmp/fon_dhcp_list");
...
static void fon_dhcp_list_add(unsigned char* mac)
{
    char conn_buf[101] = {0};
    FILE *fp;
   /* put fon clients to file */
    snprintf(conn_buf, 101,"%s", mac); 
    if ((fp = fopen("/tmp/fon_dhcp_list", "a+")) == NULL)
    {
        log_err(0, "can not open file fon_dhcp_list");
    }
    fwrite(conn_buf, strlen(conn_buf), 1, fp);
    fclose(fp);
...
int dhcp_dnsDNAT(struct dhcp_conn_t *conn, 
  ...
  if ((this->anydns ||
      ...
      tcph->dst == htons(DHCP_DNS)) {
#if(_debug_)
    log_dbg("Trace TCP DNS...");
#endif
    if (this->anydns) {
        if (
#ifdef ENABLE_LAYER3
            ...
            *do_checksum = 1;
#if(_debug_)
            log_dbg("Trace TCP ANY DNS - DNAT DNS");
#endif
        } else {
            conn->dnatdns = 0;
#if(_debug_)
            log_dbg("Trace TCP ANY DNS - without DNAT");
#endif
        }
    }
    return 1;
...
int dhcp_dnsunDNAT(struct dhcp_conn_t *conn, 
  ...
  if ((this->anydns ||
       ...
       iph->saddr == conn->dns2.s_addr) &&
      iph->protocol == PKT_IP_PROTO_TCP && 
      tcph->src == htons(DHCP_DNS)) {

    if (this->anydns && 
	conn->dnatdns &&
	iph->saddr != conn->dnatdns) {
#if(_debug_)
      log_dbg("Trace TCP DNS Response - Do SNAT");
#endif
      iph->saddr = conn->dnatdns;
      *do_checksum = 1;
    }
...
#if(_debug_)
    log_dbg("Trace TCP DNS Response");
#endif
    return 1; /* Is allowed DNS */
  }
  /* Allow TCP DNS End */
...
int dhcp_doDNAT(struct dhcp_conn_t *conn, uint8_t *pack, 
  ...
  */
  ...
  if (iph->protocol == PKT_IP_PROTO_TCP) {
    ...
    * Allow TCP DNS Start
    */
    } else if (tcph->dst == htons(DHCP_DNS)){
...
int dhcp_undoDNAT(struct dhcp_conn_t *conn, 
   ...
   * Allow TCP DNS Start
   */
  if (tcph->src == htons(DHCP_DNS) && iph->protocol == PKT_IP_PROTO_TCP) {
