...
int child_add_pid(uint8_t type, pid_t pid, char *name) {
  if (!children) {
    /* Create the list head, the main process */ children = child_create(CHILLI_PROC, getpid(), "[chilli]");
  }
...
/*
 * SC added for FON Auth Stats
 */
#if 1
void sc_nau_stats(void)
{
    FILE *fp = NULL;
    int auth_fail=0, auth_succ=0,auth_reject=0;
    char line_buf[256]; 

    if((fp = fopen("/tmp/var/hsfon_stats", "r")) != NULL)
    {
        fscanf(fp, "auth_succ=%d,auth_fail=%d,auth_reject=%d,%*s", &auth_succ, &auth_fail,&auth_reject);
        fclose(fp);
    }
    auth_reject++;
    sprintf(line_buf, "echo auth_succ=%d,auth_fail=%d,auth_reject=%d, > /tmp/var/hsfon_stats", auth_succ, auth_fail, auth_reject);
    system(line_buf);
    return;
}
#endif
