		      ...
		      int writefd)
{
	...
	if (chilloutd->clients == client) {
		...
		for (node = chilloutd->clients;
		     ((node->next != client) && (node != NULL)); node = node->next) ;
		if (node == NULL) {
	...
	if (!strcmp(argv[1], "up")) {
		...
		*/
		...
		l->redir = NULL;
	        ...
	        command_buffer[0] = '\0';
	        sprintf(command_buffer, "/usr/sbin/fonctl logout ip %s &", l->ip);
...
static int callback_redir_login(chillout_t co, void *data, int argc, char **argv)
{
	...
	if (l == NULL) {
		chillout_write(co, 0, "error address %s not registered!", argv[1]);
		return 1;
	}
	switch(l->status) {
	...
	ret = login(cd, l, argv[2], argv[3]);
        ...
        if (ret != 0)
        {
            chillout_write(co, 0, "error xl2tp fail.");
            return ret;
        }
        else
        {
...
static int free_lease_list(struct chillispot *ch)
{
	...
	for (l = ch->leases; l != NULL; l = l->next)
		free(l);
	ch->leases = NULL;
	return 0;
}
