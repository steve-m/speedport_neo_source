...
iface_init_defaults(struct Interface *iface)
{
	...
	iface->RtrSolInterval = DFLT_RtrSolInterval;
	iface->RtrSolCount = DFLT_RtrSolCount;
	iface->RtrDefaultLifetime = DFLT_RtrDefaultLifetime;
}
...
check_iface(struct Interface *iface)
{
	...
	if (iface->RtrSolInterval < 0) {
		iface->RtrSolInterval = DFLT_RtrSolInterval;
	}
	if (iface->RtrSolCount < 0) {
		iface->RtrSolCount = DFLT_RtrSolCount;
	}
	if (iface->RtrDefaultLifetime <= 0) {
		iface->RtrDefaultLifetime = DFLT_RtrDefaultLifetime;
	}
