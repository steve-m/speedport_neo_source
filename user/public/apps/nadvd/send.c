...
int
send_na(struct Interface *iface, struct in6_addr *dest)
{
	...
	struct nd_neighbor_advert *nddvert;
	struct timespec time_now;
	time_t secs_since_last_na;
	...
	secs_since_last_na = time_diff_secs(&time_now, &iface->last_na_time);
	if (secs_since_last_na < 0) {
		secs_since_last_na = 0;
		flog(LOG_WARNING, "clock_gettime() went backwards!");
	}
	iface->last_na_time = time_now;
	...
	nddvert = (struct nd_neighbor_advert *) buff;

	send_ra_inc_len(&len, sizeof(struct nd_neighbor_advert));

	nddvert->nd_na_type  = ND_NEIGHBOR_ADVERT;
	nddvert->nd_na_code  = 0;
	nddvert->nd_na_cksum = 0;

	nddvert->nd_na_flags_reserved = ND_NA_FLAG_ROUTER | ND_NA_FLAG_OVERRIDE;

	memcpy(&nddvert->nd_na_target, &iface->if_addr, sizeof(struct in6_addr));
