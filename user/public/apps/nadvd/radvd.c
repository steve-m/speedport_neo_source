...
void timer_handler_na(void *data);
void config_interface(void);
void kickoff_adverts_na(void);
void version(void);
void usage(void);
...
void write_pid_file(char const * pidfile)
{
	...
	if ((fd = open(pidfile, O_RDONLY, 0)) > 0)
	{
		...
		if (ret < 0)
		{
			flog(LOG_ERR, "cannot read nadvd pid file, terminating: %s", strerror(errno));
			exit(1);
		}
		if (ret > 0) {
				pid_t pid;
				...
				pid = strtopid(pidstr);
				if (pid > 0 && !kill(pid, 0)) {
					flog(LOG_ERR, "nadvd already running, terminating.");
					exit(1);
				}
		}
		close(fd);
	...
	if (fd < 0)
	{
		flog(LOG_ERR, "cannot create nadvd pid file, terminating: %s", strerror(errno));
		exit(1);
	}
	...
	{
		flog(LOG_ERR, "cannot write nadvd pid file, terminating: %s", strerror(errno));
		exit(1);
	}
...
void main_loop(void)
{
	...
	dlog(LOG_DEBUG, 5, "main_loop");

	for (;;) {
		struct Interface *next_na = NULL;
		struct Interface *iface;
		int timeout_na = -1;
		int rc_na = 1;

		if (IfaceList) {
			timeout_na = next_time_msec_na(IfaceList);
			next_na = IfaceList;
			for (iface = IfaceList; iface; iface = iface->next) {
				int t_na;
				t_na = next_time_msec_na(iface);
				if (timeout_na > t_na) {
					timeout_na = t_na;
					next_na = iface;
				}
		...
		dlog(LOG_DEBUG, 5, "polling for %g seconds.", timeout_na/1000.0);

		rc_na = poll(fds, sizeof(fds)/sizeof(fds[0]), timeout_na);
...
void
timer_handler_na(void *data)
{
	struct Interface *iface = (struct Interface *) data;
	double next_na = 8;

	dlog(LOG_DEBUG, 4, "timer_handler_na called for %s", iface->Name);

	send_na(iface, NULL);

	iface->next_multicast_na = next_timespec(next_na);
}
void
...
kickoff_adverts_na(void)
{
	 ...
	 */
	dlog(LOG_DEBUG, 3, "kickoff_adverts_na");

	for(iface=IfaceList; iface; iface=iface->next)
	{
		double next_na = 8;

		clock_gettime(CLOCK_MONOTONIC, &iface->last_na_time);
		clock_gettime(CLOCK_MONOTONIC, &iface->last_multicast_na);

		/* send an initial advertisement */
		if (send_na(iface, NULL) == 0) {
			iface->next_multicast_na = next_timespec(next_na);
		}
	}
}
