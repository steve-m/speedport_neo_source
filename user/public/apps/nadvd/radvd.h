...
struct Interface {
	...
	struct timespec		last_multicast_na;
	struct timespec		next_multicast;
	struct timespec		next_multicast_na;
...
int timespecdiff(struct timespec const *a, struct timespec const *b);
int next_time_msec_na(struct Interface const * iface);
...
/* send.c */
int send_na(struct Interface *iface, struct in6_addr *dest);
