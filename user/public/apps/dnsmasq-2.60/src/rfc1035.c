...
#include "dnsmasq.h"
extern struct txt_record *txt_red, *txt_head,*txt_tail;
extern struct srv_record *srv_red, *srv_head,*srv_tail;
...
#ifdef __SC_BUILD__
  int priority,weight,port;//used in srv packet
  int flag_srv = 0;
  int flag_srv_txt = 0;
#endif
...
#endif
#ifdef __SC_BUILD__
          else if(qtype == T_SRV || qtype == T_TXT)
          {
              flag_srv_txt = 1;
              addrlen = INADDRSZ;
              flags |= F_IPV4;
...
txt_loop1:
              ...
              for (; j != 0; j--) 
...
#endif
                  {
                      ...
                      {
                          if (!(res = extract_name(header, qlen, &p1, name, 1, 0)))
                              return 0;
                      }
#else
                      if (!(res = extract_name(header, qlen, &p1, name, 0, 10)))
...
#ifdef __SC_BUILD__
                      if (aqclass == C_IN && res != 2 && (aqtype == T_CNAME || aqtype == qtype) && flag_srv_txt != 1)
#else
...
#endif
                          {
                              if (aqtype == T_CNAME)
                              {
                                  if (!cname_count--)
                                      return 0; /* looped CNAMES */
#ifdef __SC_BUILD__
                                  /* when the answer just has one cname, we will not save it into cache */
                                  if(j == 1)
                                      return 0;
#endif
...
#ifdef __SC_BUILD__
                          else if (aqclass = C_IN && flag_srv_txt == 1)
                          { 
                              if(aqtype == T_SRV)
                              {
                                  flag_srv = 1;

                                  GETSHORT(priority, p1);
                                  GETSHORT(weight, p1);
                                  GETSHORT(port, p1);
                                  char tmp[MAXDNAME] = {0};
                                  extract_name(header, qlen, &p1, tmp, 1, 0);
                                  if(!(srv_tail = (struct srv_record *)malloc(sizeof( struct srv_record))))
                                  { 
                                      return 0;
                                  }
                                  if(!(srv_tail->name = (char *)malloc(MAXDNAME)))
                                  {
                                      free(srv_tail);
                                      return 0;
                                  }   
                                  if(!(srv_tail->target = (char *)malloc(MAXDNAME)))
                                  {
                                      free(srv_tail->name);
                                      free(srv_tail);
                                      return 0;
                                  }
                                  memset(srv_tail->name, 0, MAXDNAME);
                                  memset(srv_tail->target, 0, MAXDNAME);

                                  srv_tail->ttd = (time_t)(now + attl);
                                  srv_tail->priority = priority;
                                  srv_tail->weight = weight;
                                  srv_tail->port = port;
                                  srv_tail->len = ardlen;
                                  srv_tail->class = aqclass;
                                  strncpy(srv_tail->name, name, MAXDNAME);
                                  strncpy(srv_tail->target, tmp, ardlen-6);
                                  srv_red->next = srv_tail;
                                  srv_tail->next = NULL;
                                  srv_red = srv_tail;

                                  newc = cache_insert(name, NULL, now, attl, F_SRV | F_FORWARD);
                                  p1=endrr;
                              ...
                              {
                                  char *tmp = NULL;
                                  if(!(tmp = (char *) malloc(sizeof(char)*(ardlen+10))))
                                  {
                                      return 0;
                                  }
                                  snprintf(tmp,ardlen+1,"%s",p1);
                                  if(!(txt_tail = (struct txt_record *)malloc(sizeof( struct txt_record))))
                                  { 
                                      free(tmp);
                                      return 0;
                                  }
                                  if(!(txt_tail->name = (char *)malloc(MAXDNAME)))
                                  { 
                                      free(tmp);
                                      free(txt_tail);
                                      return 0;
                                  }
                                  memset(txt_tail->name, 0, MAXDNAME);
                                  if(!(txt_tail->txt = (unsigned char *)malloc(ardlen+1)))
                                  { 
                                      free(tmp);
                                      free(txt_tail->name);
                                      free(txt_tail);
                                  ...
                                  memset(txt_tail->txt, 0, ardlen+1);

                                  txt_tail->ttd = (time_t)(now + attl);
                                  txt_tail->len = ardlen;
                                  txt_tail->class = aqclass;
                                  strncpy(txt_tail->name, name, MAXDNAME);
                                  strncpy(txt_tail->txt, tmp, ardlen+1); 
                                  txt_red->next = txt_tail;
                                  txt_tail->next = NULL;
                                  txt_red = txt_tail;

                                  free(tmp);
                                  newc = cache_insert(name, NULL, now, attl, F_TXT | F_FORWARD);
                                  p1 = endrr;
                                  j--;
                                  goto txt_loop1;
...
#ifdef __SC_BUILD__
  /*start cache the additional records */
  if(ntohs(header->arcount) > 0)
  {
      unsigned char *tmp;
      unsigned long  attl;
      ...
      for (j = ntohs(header->arcount); j != 0; j--) 
      {
          ...
          {			  
              ...
              cache_insert(name, &addr, now, attl, flags | F_FORWARD);
          }
  ...
  /* end cache the additional records*/
#endif
	      ...
	      if (t->class == qclass && hostname_isequal(name, t->name))
		{
		  ...
		  if (!dryrun)
			...
			anscount++;
              ...
              if ((crecp = cache_find_by_name(NULL, name, now, F_TXT)))
              {
                      /* don't answer wildcard queries with data not from /etc/hosts
                         or DHCP leases */
                      struct txt_record *txt_tmp;
                      txt_tmp = txt_head->next;
                      while(txt_tmp!=NULL)
                      {
                          if (hostname_isequal(txt_tmp->name,name))
                              break;
                      ...
                      else if ((crecp->flags & (F_HOSTS | F_DHCP)) || !sec_reqd)
                      {
                          ...
                          if (!dryrun)
                          {
                              ...
                              while(txt_tmp != NULL && hostname_isequal(txt_tmp->name, name))
                            {
                              if (add_resource_record(header, limit, &trunc, nameoffset, &ansp, 
                                                      crec_ttl(crecp, now), NULL,
                                  T_TXT, txt_tmp->class, "t", txt_tmp->len, txt_tmp->txt))
                                      anscount++;
                              txt_tmp = txt_tmp->next;
                            }
                          }
              ...
              else
              {
                  ...
                  while(txt_tmp != NULL)
                  {
                      if (difftime(now, txt_tmp->ttd) > 0 || hostname_isequal(txt_tmp->name, name))
                      {
		...
		if (rec->issrv && hostname_isequal(name, rec->name))
		  ...
		  up = &rec->next;
...
#ifdef __SC_BUILD__
              if ((crecp = cache_find_by_name(NULL, name, now, F_SRV)))
              {   
                  /* don't answer wildcard queries with data not from /etc/hosts
                     or DHCP leases */

                  struct srv_record *srv_tmp;
                  srv_tmp = srv_head->next;
                  while(srv_tmp!=NULL)
                  {
                      if (hostname_isequal(srv_tmp->name,name))
                          break;
                  ...
                  else if ((crecp->flags & (F_HOSTS | F_DHCP)) || !sec_reqd)
                  {
                      ...
                      if (!dryrun)
                      {
                          log_query(crecp->flags & ~F_REVERSE, name, &crecp->addr.addr,
                                    record_source(crecp->uid));
                          while(srv_tmp != NULL && hostname_isequal(srv_tmp->name, name))
                          {

                              if (add_resource_record(header, limit, &trunc, nameoffset, &ansp,crec_ttl(crecp, now), 
                                    NULL, T_SRV, srv_tmp->class, "sssd",
                                    srv_tmp->priority, srv_tmp->weight, srv_tmp->port, srv_tmp->target))
                                  anscount++;
                              srv_tmp = srv_tmp->next;
                          }
                      }
              ...
              else
              {
                  ...
                  while(srv_tmp != NULL)
                  {
                      if (difftime(now, srv_tmp->ttd) > 0 || hostname_isequal(srv_tmp->name, name))
                      {
