...
static int uid = 0;
extern struct txt_record *txt_head;
extern struct srv_record *srv_head;
...
#if __SC_BUILD__
  if(daemon->qry_in_abuse && addr) {
      char addrbuf[INET6_ADDRSTRLEN];
      char cmdbuf[512];
      if((flags & F_IPV4) && (g_ea_flag & SC_EA_DO_V4)) {
          sprintf(cmdbuf, "iptables -D FWD_ABUSE_1 -d %s -j RETURN", inet_ntop(AF_INET, &addr->addr.addr4, addrbuf, sizeof(addrbuf)));
          system(cmdbuf);
          sprintf(cmdbuf, "iptables -I FWD_ABUSE_1 -d %s -j RETURN", inet_ntop(AF_INET, &addr->addr.addr4, addrbuf, sizeof(addrbuf)));
          system(cmdbuf);
      }
      else if((flags & F_IPV6) && (g_ea_flag & SC_EA_DO_V6)) {
          sprintf(cmdbuf, "ip6tables -D FWD_ABUSE_1 -d %s -j RETURN", inet_ntop(AF_INET6, &addr->addr.addr6, addrbuf, sizeof(addrbuf)));
          system(cmdbuf);
          sprintf(cmdbuf, "ip6tables -I FWD_ABUSE_1 -d %s -j RETURN", inet_ntop(AF_INET6, &addr->addr.addr6, addrbuf, sizeof(addrbuf)));
          system(cmdbuf);
      }
  }
#endif
			    ...
			    unsigned short flags, int index, struct crec **rhash, int hashsz)
{
  ...
  if (lookup && (lookup->flags & F_HOSTS))
  {
      ...
      else
      {
          if(flags & F_IPV4)
          {
              inet_ntop(AF_INET, &lookup->addr.addr, lookup_addr, ADDRSTRLEN);
              cache_del_hosts(lookup_addr, cache->name.sname,"4");
          }
          else if(flags & F_IPV6)
          {
              inet_ntop(AF_INET6, &lookup->addr.addr, lookup_addr, ADDRSTRLEN);
              cache_del_hosts(lookup_addr, cache->name.sname,"6");
          }
      }
#endif
  }
...
void dump_cache(time_t now)
{
...
#ifdef __SC_BUILD__
  FILE *cache_fp = NULL;
  if(daemon->cache_file != NULL)
      cache_fp = fopen(daemon->cache_file, "w+"); 

  if(cache_fp == NULL)
  {
      my_syslog(LOG_INFO, _("can't open cache file: %s"), daemon->cache_file);
      return;
  }

  fprintf(cache_fp, "time %lu\n", (unsigned long)now);
  fprintf(cache_fp, "cache size %d, %d/%d cache insertions re-used unexpired cache entries.\n", 
	    daemon->cachesize, cache_live_freed, cache_inserted);
  fprintf(cache_fp, "queries forwarded %u, queries answered locally %u\n", 
...
#ifdef __SC_BUILD__
	fprintf(cache_fp, "server %s#%d: queries sent %u, retried or failed %u\n", daemon->addrbuff, port, queries, failed_queries);
#else
	...
	for (cache = hash_table[i]; cache; cache = cache->hash_next)
	  {
	    ...
	    p += sprintf(p, "%-64.64s ", cache_get_name(cache));
	    ...
	    p += sprintf(p, "%-30.30s %s%s%s%s%s%s%s%s%s%s  ", a, 
			 ...
			 cache->flags & F_SRV ? "SRV" : " ",
			 cache->flags & F_TXT ? "TXT" : " ",
...
#endif
#ifdef __SC_BUILD__
  struct txt_record *txt_q;
  struct srv_record *srv_q;
  FILE *fp;
  char *temp, ctime_temp[30] = {0};
  struct crec *crecp = NULL; 

  if((fp = fopen("/tmp/var/srv_txt.log", "w+")) == NULL)
  {
      return;
  }
  txt_q = txt_head->next;
  ...
  while(txt_q != NULL)
  {
      crecp = cache_find_by_name(NULL, txt_q->name, NULL, F_TXT);
      if (crecp == NULL)
      { 
          txt_q = txt_q->next;
          continue;
      }
      sprintf(ctime_temp, "%s",  ctime(&(crecp->ttd)));
      ctime_temp[strlen(ctime_temp) - 1] = 0;
      temp = txt_q->txt;
      fprintf(fp,"%-64.64s %-3.3s %-30.30s %10d %s\n",txt_q->name, "TXT", ctime_temp, txt_q->len, ++txt_q->txt);
      txt_q->txt = temp;
      txt_q = txt_q->next;
  }
  while(srv_q != NULL)
  {
      crecp = cache_find_by_name(NULL, srv_q->name, NULL, F_SRV);
      if (crecp == NULL)
      {
          srv_q = srv_q->next;
          continue;
      ...
      sprintf(ctime_temp, "%s",  ctime(&(crecp->ttd)));
      ctime_temp[strlen(ctime_temp) - 1] = 0;
      fprintf(fp,"%-64.64s %-3.3s %-30.30s %10d %s\n",srv_q->name, "SRV", ctime_temp, srv_q->len, srv_q->target);
      srv_q = srv_q->next;
  }
  fclose(fp);
...
#ifdef __SC_BUILD__
void cache_add_hosts(char *ip_addr, char *hostname)
{
    /*update networkmap hosts*/
    struct crec *cache;
    int revhashsz;
    struct all_addr addr;
    unsigned short flags = 0;
    ...
    else if (inet_pton(AF_INET6, ip_addr, &addr) > 0)
    {
        flags = F_HOSTS | F_IMMORTAL | F_FORWARD | F_REVERSE | F_IPV6;
        addrlen = IN6ADDRSZ;
    }
    ...
    /* add hosts to cache */
    if ((cache = whine_malloc(sizeof(struct crec) + strlen(hostname)+1-SMALLDNAME)))
    {
        strcpy(cache->name.sname, hostname);
        add_hosts_entry(cache, &addr, addrlen, flags, 1, (struct crec **)daemon->packet, revhashsz);
    }
}
void cache_del_hosts(char *ip_addr,char *hostname, char *ipversion)
{
    ...
    my_syslog(LOG_INFO,"hostname is %s, ipaddr is %s, ipversion is %s\n",hostname,ip_addr, ipversion);
    for (i=0; i<hash_size; i++)
        for (cache = hash_table[i], up = &hash_table[i]; cache; cache = tmp)
        {
            tmp = cache->hash_next;
            if (cache->flags & F_HOSTS)
            {
                if((hostname_isequal(cache->name.sname,hostname) == 1 || hostname_isequal(cache->name.sname,hostname_suffix) == 1) && (flag & cache->flags))
                {
                    if(flag & F_IPV4)
                        inet_ntop(AF_INET, &cache->addr.addr, tmp_addr, ADDRSTRLEN);
                    else
                        inet_ntop(AF_INET6, &cache->addr.addr, tmp_addr, ADDRSTRLEN);

                    if(strcmp(ip_addr, tmp_addr) == 0)
                    {
                        *up = cache->hash_next;
                        free(cache);
                    }
