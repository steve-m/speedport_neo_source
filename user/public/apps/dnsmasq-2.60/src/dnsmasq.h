...
#ifdef __SC_BUILD__
#define P008_IPV6_SYSLOG_NUM (10)
#define P008_SYSLOG_TIME_INTERVAL (300) /* seconds */
#endif
...
#ifdef __SC_BUILD__
  /*
   * retry_new_id[0][0] -> 1st pppv6_pri
   * retry_new_id[0][1] -> 2nd pppv6_pri
   * retry_new_id[0][2] -> 2nd pppv6_sec
   * retry_new_id[0][3] -> 3th pppv4_pri
   * retry_new_id[1][0] -> 4th pppv4_pri
   * retry_new_id[1][1] -> 4th pppv4_sec
   * retry_new_id[1][2] -> 5th dhcpv4_pri
   * retry_new_id[1][3] -> 5th dhcpv4_sec
   */
  unsigned short retry_new_id[2][4];
  struct all_addr *orig_addrp;
  size_t orig_size;
  char orig_name[MAXDNAME],new_name[MAXDNAME];
  int pppv4_send;
  char dns_server[3][50];
  int ipv6_flag ;
  int dnsv6_pri_reply;
  int dnsv6_sec_reply;
  int lastserverflag;/* used for everydnsserver send error message */
  int err_code;
  int single_dns;
  unsigned int hash;
#endif
  int fd, forwardall, flags, plen;
  int s_type;
  unsigned int crc;
  time_t time;
  struct timeval tv;
...
#ifdef __SC_BUILD__
  char *cache_file; /* optional cache file */
#endif
...
int extract_name(struct dns_header *header, size_t plen, unsigned char **pp, char *name, int isExtract, int extrabytes);
#endif
