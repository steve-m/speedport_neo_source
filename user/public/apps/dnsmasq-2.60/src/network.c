  ...
  for (new = daemon->servers; new; new = tmp)
      ...
      if (!(new->flags & SERV_NO_REBIND))
	{
	  if (new->flags & (SERV_HAS_DOMAIN | SERV_FOR_NODOTS | SERV_USE_RESOLV))
	    ...
	    my_syslog(LOG_INFO, _("using nameserver %s#%d as %s"), daemon->namebuff, port, new->s_type == 1?"pppv6_pri":new->s_type == 2?"pppv6_sec":new->s_type == 3?"pppv4_pri":new->s_type == 4?"pppv4_sec":new->s_type == 5?"dhcpv4_pri":new->s_type == 6?"dhcpv4_sec":"unknow"); 
	}
    }
  ...
  while ((line = fgets(daemon->namebuff, MAXDNAME, f)))
    {
      ...
      my_syslog(LOG_WARNING, _("read %s from conf"),line );
      if (!token)
	continue;
      my_syslog(LOG_WARNING, _("parse %s from conf"),token );
      if (strcmp(token, "nameserver_pppv6_pri") == 0)
          s_type = 1;
      else if (strcmp(token, "nameserver_pppv6_sec") == 0)
          s_type = 2;
      else if (strcmp(token, "nameserver_pppv4_pri") == 0)
          s_type = 3;
      else if (strcmp(token, "nameserver_pppv4_sec") == 0)
          s_type = 4;
      else if (strcmp(token, "nameserver_dhcpv4_pri") == 0)
          s_type = 5;
      else if (strcmp(token, "nameserver_dhcpv4_sec") == 0)
          s_type = 6;
      else
	continue;
      my_syslog(LOG_WARNING, _("read type %d from conf"),s_type );
      if (!(token = strtok(NULL, " \t\n\r")))
	continue;
      
      my_syslog(LOG_WARNING, _("read addr %s from conf"),token );
      memset(&addr, 0, sizeof(addr));
