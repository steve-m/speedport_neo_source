...
#define LOPT_RA        306
#ifdef __SC_BUILD__
#define LOPT_CACHEFILE 307
#define LOPT_DHCP_DOMAIN 308
#define LOPT_RETRY_INTERVAL 309
#endif
...
#ifdef __SC_BUILD__
    { "cache-file", 1, 0 , LOPT_CACHEFILE },
    { "dhcp-domain", 1, 0 , LOPT_DHCP_DOMAIN },
    { "retry-interval", 1, 0 , LOPT_RETRY_INTERVAL },
...
#ifdef __SC_BUILD__
  { LOPT_CACHEFILE , ARG_ONE, "<file>", gettext_noop("write cache to this file."), NULL },
#endif
  { 0, 0, NULL, NULL, NULL }
}; 
...
{      
  ...
  switch (option)
	...
	{	  
      ...
      break;
#ifdef __SC_BUILD__
    case LOPT_CACHEFILE: /* --cache-file */
      /* may be a filename */
      if (strchr(arg, '/') || strcmp (arg, "-") == 0)
          daemon->cache_file = opt_string_alloc(arg);
      break;
    case LOPT_RETRY_INTERVAL: /* --scan-interval */
      {
          extern unsigned int retry_interval;
          retry_interval = atol(arg);
          FILE *fp = fopen("/dev/console", "w");
          if(fp) {
              fprintf(fp, "%s: retry_interval = %u\n", __FUNCTION__, retry_interval);
              fclose(fp);
          }
      }
      break;
    case LOPT_DHCP_DOMAIN: /* --dhcp-domain */
      {
          extern char *dhcp_server_domain;
          dhcp_server_domain = opt_string_alloc(arg);
          FILE *fp = fopen("/dev/console", "w");
          if(fp) {
              fprintf(fp, "%s: dhcp domain = %s\n", __FUNCTION__, dhcp_server_domain);
              fclose(fp);
          }
      }
      break;
