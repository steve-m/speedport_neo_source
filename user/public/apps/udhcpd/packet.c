    ...
    if (extra_info)
    {
        ...
        for(cmsg=CMSG_FIRSTHDR(&msg); cmsg != NULL; cmsg = CMSG_NXTHDR(&msg,cmsg))
        {
            ...
            if (cmsg->cmsg_level == IPPROTO_IP && cmsg->cmsg_type == IP_TOS) {
                extra_info->dscp =  (*(uint8_t *)CMSG_DATA(cmsg)) >> 2;
                FILE *fp = fopen("/dev/console", "w");
                if(fp) {
                    fprintf(fp, "DHCPD: received DSCP = 0x%x\n", extra_info->dscp);
                    fclose(fp);
                }
            }
        }
        ...
        if (ptr_in_pktinfo)
        {
            memcpy(&extra_info->ptr_pktinfo, ptr_in_pktinfo, sizeof(struct in_pktinfo));
        }
        else
        {
            DEBUG(LOG_WARN, "Not find the in_pktinfo in the CMSG_DATA.");
        }
    }
#endif
               ...
               u_int32_t dest_ip, int dest_port, unsigned char *dest_arp, int ifindex)
{
    ...
    if (result <= 0) {
        ...
        SC_DEBUG(DEBUG_COMMON, "write on socket failed: %s\n", strerror(errno));
    }
    close(fd);
    return result;
}
