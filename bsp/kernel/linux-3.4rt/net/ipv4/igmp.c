...
#define IP_MAX_MEMBERSHIPS	34 //DT requirement, max group is 32
...
#define IGMP_Unsolicited_Report_Interval	(1*HZ) //DT requirement for report interval in 1s
	...
	int len)
{
	...
	if (len == 8) {
        /* Put IGMP v1/v2 Query to MCPD for statistic*/
        sc_mcpd_process_skb(in_dev, skb, 0);
		if (ih->code == 0) {
        ...
        /* Put the IGMPv3 WAN Query packet to MCOD, DT need sync
         * WAN query interval to LAN
         * */
        sc_mcpd_process_skb(in_dev, skb, 0);
