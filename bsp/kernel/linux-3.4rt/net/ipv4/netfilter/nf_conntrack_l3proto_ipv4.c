...
out:
        ...
        do {
            ...
            if(ct && CTINFO2DIR(ctinfo) == IP_CT_DIR_ORIGINAL && !nf_ct_is_dying(ct))
            {
                if(!ct->sc_out_dev) {
                    ct->sc_out_dev = skb_dst(skb)->dev;
                    break;
                }

                if(skb_dst(skb)->dev != ct->sc_out_dev &&
                   strcmp(skb_dst(skb)->dev->name, "lo") != 0
                   )
                {
                    printk("%s:output device changed, skb->dev = %s, ct->dev = %s\n",
                           __FUNCTION__,
