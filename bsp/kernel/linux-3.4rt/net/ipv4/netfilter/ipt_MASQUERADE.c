...
static int port_restricted_nat_help(struct sk_buff *skb, unsigned int protoff,
                                    struct nf_conn *ct, enum ip_conntrack_info ctinfo)
{
	int dir = CTINFO2DIR(ctinfo);
	struct nf_conn_help *help = nfct_help(ct);
	...
	nf_ct_expect_init(exp, NF_CT_EXPECT_CLASS_DEFAULT, AF_INET,
			  ...
			  &ct->tuplehash[!dir].tuple.dst.u3, nf_ct_protonum(ct),
...
static struct nf_conntrack_helper nf_conntrack_helper_port_restricted_nat __read_mostly = {
	...
	.expect_policy = &bcm_nat_exp_policy,
	.expect_class_max = 1,
	.help = port_restricted_nat_help,
};
static struct nf_conntrack_helper nf_conntrack_helper_port_restricted_nat_tcp __read_mostly = {
	.name = "BCM-NAT",
	.me = THIS_MODULE,
	.tuple.src.l3num = AF_INET,
...
/****************************************************************************/
#if defined(_SC_BUILD_)
static inline int find_exp(__be32 ip, __be16 port, __be16 dst_port, __be32 dst_addr, struct nf_conn *ct)
#else
...
#endif
{
	...
	tuple.src.l3num = AF_INET;
#if defined(_SC_BUILD_)
	tuple.src.u.all = dst_port;
	tuple.src.u3.ip = dst_addr;
	tuple.dst.protonum = nf_ct_protonum(ct);
#else
	tuple.dst.protonum = IPPROTO_UDP;
#endif
	tuple.dst.u3.ip = ip;
...
#if defined(_SC_BUILD_)
static inline struct nf_conntrack_expect *find_port_restricted_exp(struct nf_conn *ct)
{
	struct nf_conntrack_tuple * tp =
		&ct->tuplehash[IP_CT_DIR_ORIGINAL].tuple;
	struct net *net = nf_ct_net(ct);
	...
	for (h = 0; h < nf_ct_expect_hsize; h++) {
		hlist_for_each_entry_rcu(i, n, &net->ct.expect_hash[h], hnode) {
			if (i->saved_ip == tp->src.u3.ip &&
		    	    i->saved_proto.all == tp->src.u.all &&
		    	    i->tuple.dst.protonum == tp->dst.protonum &&
		    	    (i->tuple.src.u3.ip == tp->dst.u3.ip || i->tuple.src.u3.ip == 0) &&
		    	    (i->tuple.src.u.all == tp->dst.u.all || i->tuple.src.u.udp.port == 0)) {
				exp = i;
				break;
			}
		}
...
#if defined(CONFIG_BCM_KF_NETFILTER)
#if defined(_SC_BUILD_) && defined(_SC_CONE_NAT_TCP_)
...
#if defined(_SC_BUILD_)
                int port_restricted = (mr->range[0].min_ip == 2);
                __be16 dst_port = 0;
                __be32 dst_addr = 0;
                if(port_restricted) {
                        dst_port = ct->tuplehash[IP_CT_DIR_ORIGINAL].tuple.dst.u.all;
                        dst_addr = ct->tuplehash[IP_CT_DIR_ORIGINAL].tuple.dst.u3.ip;
                }
#endif
...
#if defined(_SC_BUILD_)
                if(port_restricted)
                    exp = find_port_restricted_exp(ct);
                else
#endif
...
#if defined(_SC_BUILD_)
			     	if (!find_exp(newsrc, htons(newport), dst_port, dst_addr, ct)) {
			...
			if (help != NULL) {
                                ...
                                if(port_restricted) {
                                    if(nf_ct_protonum(ct) == IPPROTO_UDP)
                                        help->helper = &nf_conntrack_helper_port_restricted_nat;
#if defined(_SC_CONE_NAT_TCP_)
                                    if(nf_ct_protonum(ct) == IPPROTO_TCP)
                                        help->helper = &nf_conntrack_helper_port_restricted_nat_tcp;
#endif
                                } else {
                                    if(nf_ct_protonum(ct) == IPPROTO_UDP)
...
static void __exit masquerade_tg_exit(void)
{
	...
	nf_conntrack_helper_unregister(&nf_conntrack_helper_port_restricted_nat_tcp);
#endif
#endif
