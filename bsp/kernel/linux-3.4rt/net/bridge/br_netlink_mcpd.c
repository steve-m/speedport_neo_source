...
#if _SC_BUILD_
/* Sercomm define this value must same with mcpd application */
//#define MCPD_MSG_IGMP_VLAN_ID (MCPD_MSG_MAX + 0x1000)
#if 1 //add for GARD-16820
#define MCPD_MSG_IGMP_QUERY_INTERVAL (MCPD_MSG_MAX + 0x1001)
#define MCPD_MSG_IGMP_ROBUST_VAL (MCPD_MSG_MAX + 0x1002)
#endif
#endif
...
int sc_mcpd_process_skb(struct in_device *in_dev, struct sk_buff *skb, int protocol)
{
    struct nlmsghdr *nlh;
    char *ptr = NULL;
    ...
    iph = ip_hdr(skb);
    iphdr_len = iph->ihl * 4;
    if(iphdr_len < 5 || (*(u8 *)iph >> 4) != 4) 
        return 0;
    
    len = skb->len + iphdr_len;
    pData = skb->data;
    ...
    memcpy(pkt_info->br_name, in_dev->dev->name, IFNAMSIZ);
    memset(pkt_info->port_name, 0, IFNAMSIZ);
#if defined(CONFIG_MIPS_BRCM) && defined(CONFIG_BR_MLD_SNOOP)
    memset(pkt_info->srcMac, 0, 6);
#endif
    pkt_info->port_no = -1;
    pkt_info->if_index = in_dev->dev->ifindex;
    ...
    memcpy(ptr, (u8 *)iph, iphdr_len);
    memcpy(ptr + iphdr_len, pData, len - iphdr_len);

    NETLINK_CB(new_skb).dst_group = 0;
    NETLINK_CB(new_skb).pid = mcpd_pid;
    
    //sc_mcpd_dump_buf((char *)nlh, 128);
    netlink_unicast(nl_sk, new_skb, mcpd_pid, MSG_DONTWAIT);

    return 1;
} /* sc_mcpd_process_skb */
EXPORT_SYMBOL(sc_mcpd_process_skb);
...
int mcpd_process_skb(struct net_bridge *br, struct sk_buff *skb, unsigned short protocol)
{
    ...
    pkt_info->lanppp = lanppp;
#ifdef _SC_BUILD_
    /* Only sync struct pkt_info between kernel and usersapce
     * The hole pkt will send to userspace(include ip hdeader),
     * get tos value in userspace
     * */
    pkt_info->tos = 0;
#endif
...
#if _SC_BUILD_
/*static void mcpd_nl_process_vlan_id(struct sk_buff *skb)
{
    struct nlmsghdr *nlh = (struct nlmsghdr *)skb->data;
    unsigned char   *ptr;
    int val;
    ...
    br_igmp_proxy_vlan_id_update(val);

    return;
}*/
#if 1 //add for GARD-16820
static void mcpd_nl_proccess_query_interval(struct sk_buff *skb)
{
    struct nlmsghdr *nlh = (struct nlmsghdr *)skb->data;
    unsigned char   *ptr;
    int val;
    ...
    br_igmp_proxy_query_interval_update(val);

    return;

}
static void mcpd_nl_proccess_robust_val(struct sk_buff *skb)
{
    struct nlmsghdr *nlh = (struct nlmsghdr *)skb->data;
    unsigned char   *ptr;
    int val;
    ...
    br_igmp_proxy_robust_val_update(val);
...
#if _SC_BUILD_
	    /*case MCPD_MSG_IGMP_VLAN_ID:
                mcpd_nl_process_vlan_id(skb);
		break;*/

#if 1 //add for GARD-16820
        case MCPD_MSG_IGMP_QUERY_INTERVAL:
                mcpd_nl_proccess_query_interval(skb);
        break;

        case MCPD_MSG_IGMP_ROBUST_VAL:
                mcpd_nl_proccess_robust_val(skb);
        break;
#endif
#endif
