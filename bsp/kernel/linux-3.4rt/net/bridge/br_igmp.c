...
#if _SC_BUILD_
//static int br_igmp_proxy_vlan_id = 8;
#if 1 //add for GARD-16820
static int br_igmp_proxy_query_interval = 60;
static int br_igmp_proxy_robust_val = 2;
#endif
#endif
...
#if _SC_BUILD_
/*void br_igmp_proxy_vlan_id_update(int val)
{
    br_igmp_proxy_vlan_id = val;
}*/
#if 1 //add for GARD-16820
void br_igmp_proxy_query_interval_update(int val)
{
    br_igmp_proxy_query_interval = val;
}
void br_igmp_proxy_robust_val_update(int val)
{
    br_igmp_proxy_robust_val = val;
}
#endif
/*int br_igmp_get_proxy_vlan_id_info(void)
{
    return br_igmp_proxy_vlan_id;
}*/
#endif
...
#if 0 //change for GARD-16820
	unsigned long                   tstamp = jiffies + (BR_IGMP_MEMBERSHIP_TIMEOUT*HZ*2);
#else	
    unsigned long                   tstamp = jiffies + ((br_igmp_proxy_query_interval * br_igmp_proxy_robust_val + 10)*HZ*2);
#endif
...
#else
						rep_entry->tstamp = jiffies + (br_igmp_proxy_query_interval * br_igmp_proxy_robust_val + 10)*HZ;
#endif
...
#else
					reporter->tstamp = jiffies + (br_igmp_proxy_query_interval * br_igmp_proxy_robust_val + 10)*HZ;
#endif
					br_igmp_set_timer(br);
				}
				ret = 1;
...
/*#if _SC_BUILD_
	seq_printf(seq, "igmp snooping %d  proxy %d  lan2lan-snooping %d/%d, vlan-id %d, rate-limit %dpps, priority %d\n",
	           br->igmp_snooping, 
	           br->igmp_proxy,
	           br->igmp_lan2lan_mc_enable,
	           br_mcast_get_lan2lan_snooping(BR_MCAST_PROTO_IGMP, br),
	           br_igmp_proxy_vlan_id,
	           br->igmp_rate_limit,
