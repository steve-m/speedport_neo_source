...
#ifdef _SC_BUILD_
void (*pnf_conntrack_flush)(struct net *net) = NULL;
EXPORT_SYMBOL_GPL(pnf_conntrack_flush);
#endif
...
#endif
#ifdef _SC_BUILD_
	if (!list_empty(&ct->port_triggers)) {
                extern void trigger_timeout(unsigned long ul_trig);
		xt_porttrig *child, *tmp;
		list_for_each_entry_safe(child, tmp, &ct->port_triggers, master_ct) {
                    trigger_timeout((unsigned long)child);
		}
	}
#endif
...
#else
	if (nf_conntrack_max &&
	    unlikely(atomic_read(&net->ct.count) > nf_conntrack_max-100)) {
            if(blog_iq(skb) == IQOS_PRIO_HIGH &&
               likely(atomic_read(&net->ct.count) < nf_conntrack_max))
                goto do_alloc;
#endif
#if defined(CONFIG_BCM_KF_NETFILTER)
...
do_alloc:
	...
	*(unsigned long *)(&ct->tuplehash[IP_CT_DIR_REPLY].hnnode.pprev) = hash;
	...
	if (zone) {
        ...
        ct->no_sipalg = 0;
        ct->sc_out_dev = NULL;
        INIT_LIST_HEAD(&ct->port_triggers);
#endif
	/*
...
#ifdef _SC_BUILD_
                /*
                 * If the master of a expectation's mark is 0x3030, then the
                 * expectation is RTP and master is SIP.
                 * We set the conntrack of RTP session to 0x2E2E.
                 *
                 * Note: 0x3030 and 0x2E2E is choosen from that the DSCP value
                 * for SIP packet is 0x30 and RTP is 0x2E.
                 */
                if(exp->master->mark == 0x3030)
                    ct->mark = 0x2E2E; //DTCR 2016-370: fix the mark from 0x2828 to 0x2E2E
                else
...
#ifdef _SC_BUILD_
static inline void sc_qos_l3_classify(struct sk_buff *skb,
                                      struct nf_conntrack_tuple *tuple)
{
        if(tuple->dst.protonum == IPPROTO_UDP) {
                if((tuple->dst.u.udp.port == htons(47806)) ||
                   (tuple->src.u.udp.port == htons(47806)) ||
                   (tuple->dst.u.udp.port == htons(43962)) ||
                   (tuple->src.u.udp.port == htons(43962)))
                {
#if defined(CONFIG_MIPS_BRCM)
                        if(skb->blog_p)
                                skb->blog_p->iq_prio = IQOS_PRIO_HIGH;
#endif
                }
        }
}
...
#ifdef _SC_BUILD_
        if(pnf_conntrack_flush != NULL)
	{
		pnf_conntrack_flush(&init_net);
	}
#endif
...
#ifdef _SC_BUILD_
EXPORT_SYMBOL_GPL(get_next_corpse);
#endif
