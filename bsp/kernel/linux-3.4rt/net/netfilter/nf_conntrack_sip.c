...
/* fix rport wrong issue in via in sip header */
#if 0
                if (*dptr == '\r' || *dptr == '\n') {
		       ...
		       char **dptr, int *dlen)
{
	...
	while (find_field(&p, *dptr + *dlen, POS_VIA)) {
		 ...
		 * set by some 'smart' UAs. We need to change the port. */ 
			 ...
			 (nf_nat_addr = rcu_dereference(nf_nat_addr_hook))) {
#ifdef _SC_BUILD_
                        if(addr.u.udp.port != ct->tuplehash[dir].tuple.src.u.udp.port) {
                            printk("%s: Not change port different with UDP source.\n", __FUNCTION__);
                            ct->lan2wan_mark |= 0x10;
                            break;
                        } else {
                            ct->lan2wan_mark &= ~0x10;
                        }

                        ct->lan2wan_mark |= 0x01;
#endif
			addr.u3 = ct->tuplehash[!dir].tuple.dst.u3;
			addr.u.all = ct->tuplehash[!dir].tuple.dst.u.all;
			ret = nf_nat_addr(skb, protoff, ct, ctinfo, dptr,
		...
		/* WAN->LAN. Change the WAN address to LAN address */ 
			 ...
			 (nf_nat_addr = rcu_dereference(nf_nat_addr_hook))) {
#ifdef _SC_BUILD_
                        if(ct->lan2wan_mark & 0x10) {
                            char buffer[sizeof("nnn.nnn.nnn.nnn:nnnnn")];
                            unsigned int buflen;
                            unsigned int matchoff, matchlen, val;
                            printk("%s: WAN->LAN packet with 0x10, trying search rport.\n", __FUNCTION__);
                            if(ct_sip_parse_numerical_param(ct, *dptr, 0, *dlen, "rport=", &matchoff, &matchlen, &val) != 0) {
                                printk("%s: rport found, change rport %u -> %u\n", __FUNCTION__, val, ntohs(addr.u.udp.port));
                                buflen = sprintf(buffer, "%u", ntohs(addr.u.udp.port));
                                nf_nat_mangle_udp_packet(skb, ct, ctinfo, matchoff, matchlen, buffer, buflen);
                            }
                            break;
                        }
#endif
...
#ifdef _SC_BUILD_
	while(find_field(&p, *dptr+*dlen, POS_CONTACT)) {
#else
	 ...
	 * set by some 'smart' UAs. We need to change the port. */ 
		 ...
		 (nf_nat_addr = rcu_dereference(nf_nat_addr_hook))) {
                ...
                if(addr.u.udp.port != ct->tuplehash[dir].tuple.src.u.udp.port) {
                        printk("%s: Not change port different with UDP source.\n",
                               __FUNCTION__);
                        ct->lan2wan_mark |= 0x20;
                        break;
                } else {
                        ct->lan2wan_mark &= ~0x20;
                }
                ct->lan2wan_mark |= 0x02;
#endif
		addr.u3 = ct->tuplehash[!dir].tuple.dst.u3;
		addr.u.all = ct->tuplehash[!dir].tuple.dst.u.all;
		ret = nf_nat_addr(skb, protoff, ct, ctinfo, dptr, dlen, &p,
        ...
        /* WAN->LAN. Change the WAN address to LAN address */ 
                 ...
                 (nf_nat_addr = rcu_dereference(nf_nat_addr_hook))) {
            addr = ct->tuplehash[!dir].tuple.src;
            if(ct->lan2wan_mark&0x20)
                break;
            if(ct->lan2wan_mark&0x02)
                addr.u3 = ct->tuplehash[dir].tuple.dst.u3;
            ret = nf_nat_addr(skb, protoff, ct, ctinfo, dptr,
                              dlen, &p, &len, &addr);
		    ...
		    enum ip_conntrack_info ctinfo)
{
...
#ifdef _SC_BUILD_
        /*Fix bug: SIP ALG does not work when SIP packets fragmented.*/
	...
	if (datalen < sizeof("SIP/2.0 200") - 1)
        ...
        do {
            if(ct->no_sipalg) {
                printk("SIP ALG: no_sipalg flag set, skip process.\n");
                goto PROCESS_SDP;
            }
            ...
            if(strstr(dptr, "User-Agent: Jitsi") != NULL) {
                printk("Jitsi client INVITE found, skip SIP alg.\n");
                ct->no_sipalg = 1;
                goto PROCESS_SDP;
            }
