...
#ifdef _SC_BUILD_
#include <sc/log_api.h>
#include <sc/log_errcode_de.h>
#endif /* _SC_BUILD_ */
...
#if _SC_BUILD_
        if(strncmp(loginfo->prefix, "[EmailAbuse]", sizeof("[EmailAbuse]")-1) == 0) {
            static atomic_t dt_ea_drop_count = ATOMIC_INIT(0);
            if(atomic_inc_return(&dt_ea_drop_count) % 5 == 0) {
                if (par->family == NFPROTO_IPV4) {
                    struct iphdr _iph;
                    const struct iphdr *ih;
                    ih = skb_header_pointer(skb, 0, sizeof(_iph), &_iph);
                    if (ih != NULL) {
                        char saddr[sizeof("xxx.xxx.xxx.xxx")];
                        char daddr[sizeof("xxx.xxx.xxx.xxx")];
                        sprintf(saddr, "%pI4", &ih->saddr);
                        sprintf(daddr, "%pI4", &ih->daddr);
                        printk(KERN_INFO "%s"LOG_ERRCODE_E005"\n", log_type_db[GENERAL_LOG], saddr, daddr);
                    }
                    ...
                    if (ih != NULL) {
                        char saddr[64];
                        char daddr[64];
                        sprintf(saddr, "%pI6", &ih->saddr);
                        sprintf(daddr, "%pI6", &ih->daddr);
                        printk(KERN_INFO "%s"LOG_ERRCODE_E005"\n", log_type_db[GENERAL_LOG], saddr, daddr);
                    }
                }
            }
