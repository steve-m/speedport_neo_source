...
#ifdef _SC_BUILD_
static int sc_udp_packet(struct nf_conn *ct,
                         ...
                         unsigned int hooknum)
{
    ...
    if((tuple->dst.u.udp.port == htons(53)) ||
       (tuple->src.u.udp.port == htons(53)) ||
       (tuple->dst.u.udp.port == htons(67)) ||
       (tuple->src.u.udp.port == htons(67))
       )
    {
        nf_ct_refresh_acct(ct, ctinfo, skb, 3 * HZ);
...
#ifdef _SC_BUILD_
    if(sc_udp_packet(ct, skb, dataoff, ctinfo, pf, hooknum))
        return NF_ACCEPT;
#endif
