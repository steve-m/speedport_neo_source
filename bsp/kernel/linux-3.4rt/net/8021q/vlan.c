...
#if defined(CONFIG_BCM_KF_VLAN) && (defined(CONFIG_BCM_VLAN) || defined(CONFIG_BCM_VLAN_MODULE))
        /*
         * For linux vconfig can co-work with broadcom FAP. We need fill vlan
         * device's path structure like broadcom vlanctl.
         */
        new_dev->path = real_dev->path;
        new_dev->path.next_dev = real_dev;
        new_dev->path.refcount = 0;
#endif
#endif
