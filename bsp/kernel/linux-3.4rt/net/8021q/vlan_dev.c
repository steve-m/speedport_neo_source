...
vlan_dev_get_egress_qos_mask(struct net_device *dev, struct sk_buff *skb)
{
        ...
        struct {
                ...
                { 0x30, (6 << 13) & 0xE000 }, /* Voice Control, DNS */
                { 0x2E, (6 << 13) & 0xE000 }, /* Voice Bearer */
                { 0x20, (4 << 13) & 0xE000 }, /* MSTV Multicast, VoD, NTP */
                { 0x18, (3 << 13) & 0xE000 }, /* MSTV Unicast */
                { 0x00, (0 << 13) & 0xE000 }, /* Best Effort */
        };

        if (veth->h_vlan_proto == htons(ETH_P_8021Q))
            nh = skb->data + VLAN_HLEN;
        else
            nh = skb->data + ETH_HLEN;

        switch(ntohs(veth->h_vlan_proto)) {
        case 0x8864:
                /* PPPoE */
                pt = (u16 *)(nh + 6);
                switch(ntohs(*pt)) {
                case 0x0021:
                         ...
                         */
                        nh = (u8*)(pt + 1);
                        dscp = ipv4_get_dsfield((struct iphdr *)nh) >> 2;
                        break;
                case 0x0057:
                        /*
                         * IPv6 in PPPoE
                         */
                        nh = (u8*)(pt + 1);
                        dscp = ipv6_get_dsfield((struct ipv6hdr *)nh) >> 2;
                        break;
                case 0xc021:
                        /*
                         * LCP in PPPoE
                         * */
                        dscp = 0x30; //LCP don't have the dscp, but we should set the vlan_qos to 6
                        break;
                default:
                        return 0;
                }
        ...
        for(i = 0; i < (sizeof(dscp2vlan)/sizeof(dscp2vlan[0])); ++i)
                if(dscp == dscp2vlan[i].dscp) {
                        return dscp2vlan[i].vlan_qos;
                }
...
#error "For project F1, _SC_BUILD_ should be defined!"
