...
#ifdef _SC_BUILD_
static int proc_read_fw_version(char *page, char **start, off_t off, int cnt, int *eof, void *data);
static int proc_write_fw_version(struct file *f, const char *buf, unsigned long cnt, void *data);
static int proc_read_csn(char *page, char **start, off_t off, int cnt, int *eof, void *data);
static int proc_write_csn(struct file *f, const char *buf, unsigned long cnt, void *data);
static int proc_read_wan_mode(char *page, char **start, off_t off, int cnt, int *eof, void *data);
static int proc_write_wan_mode(struct file *f, const char *buf, unsigned long cnt, void *data);
#endif
...
{
#ifdef Entry2
#define DSL_LINK_LED 10
#define INTERNET_LED 11
    if(ledName == kLedAdsl)
    {
        switch(ledState)
        {
        ...
        case kLedStateFail:
            if(sc_led_off_hook)
            {
                printk("sc_led_off_hook_off\n");
                sc_led_off_hook(DSL_LINK_LED);
                sc_led_off_hook(INTERNET_LED);
            }
            break;
        ...
        case kLedStateFastBlinkContinues:
            if(sc_led_off_hook)
            {
                printk("sc_led_off_hook_fast\n");
                sc_led_off_hook(DSL_LINK_LED);
            }
            if(sc_led_blink_always_hook)
            {
                printk("sc_led_blink_hook_fast\n");
                sc_led_blink_always_hook(DSL_LINK_LED, 1, 25, 25, 0);
            }
            break;
        case kLedStateOn:
            if(sc_led_on_hook)
            {
                printk("sc_led_on_hook_on\n");
                sc_led_on_hook(DSL_LINK_LED);
            }
            break;
        default:
            break;
...
#define SC_CSN_BUFFER_LEN (32)
char board_fw_version[SC_FW_VERSION_BUFFER_LEN]="01.00.023";
char board_csn[SC_CSN_BUFFER_LEN]="Speedport Simplicity";
#endif
...
#define offset(type, elem) ((int)&((type *)0)->elem)
...
#ifdef _SC_BUILD_
    p3 = create_proc_entry("FW_VERSION", 0644, p0);
    if (p3 == NULL)
    {
        printk("add_proc_files: failed to create proc files:FW_VERSION!\n");
        return -1;
    }

    p3->read_proc   = proc_read_fw_version;
    p3->write_proc  = proc_write_fw_version;
#if LINUX_VERSION_CODE >= KERNEL_VERSION(2,6,30)
    //New linux no longer requires proc_dir_entry->owner field.
#else
    ...
    p4 = create_proc_entry("CSN", 0644, p0);
    if (p4 == NULL)
    {
        printk("add_proc_files: failed to create proc files:CSN!\n");
        return -1;
    }

    p4->read_proc   = proc_read_csn;
    p4->write_proc  = proc_write_csn;
#if LINUX_VERSION_CODE >= KERNEL_VERSION(2,6,30)
    //New linux no longer requires proc_dir_entry->owner field.
#else
    ...
    p5 = create_proc_entry("WAN_MODE", 0644, p0);
    if (p5 == NULL)
    {
        printk("add_proc_files: failed to create proc files:WAN_MODE!\n");
        return -1;
    }

    p5->read_proc   = proc_read_wan_mode;
    p5->write_proc  = proc_write_wan_mode;
#if LINUX_VERSION_CODE >= KERNEL_VERSION(2,6,30)
    //New linux no longer requires proc_dir_entry->owner field.
#else
    p5->owner       = THIS_MODULE;
#endif
...
{
    int len = 0;	
    len += sprintf(page+len, "%s", board_fw_version);
    return len;
...
{
    int len = 0;	
    len += sprintf(page+len, "%s", board_csn);
    return len;
}
...
{
    int len = 0;
    len += sprintf(page+len, "%d", board_wan_mode);
    return len;
}
...
{
    int ret = cnt;
    char board_wan_mode_buf[5]="";

    sprintf(board_wan_mode_buf,"%d",board_wan_mode);

    if (cnt > sizeof(board_wan_mode_buf))
        return -EFAULT;

    if (copy_from_user(board_wan_mode_buf, buf, cnt))
    {
        return -EFAULT;
    }
   
    board_wan_mode=board_wan_mode_buf[0]-'0';
...
#ifdef _SC_BUILD_
static int user_mode_reboot_flag = 0;
#endif
