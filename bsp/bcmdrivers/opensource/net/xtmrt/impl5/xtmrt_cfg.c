...
{
	int ifindex;
	char defMac[NVRAM_MAC_ADDRESS_LEN];
...
#ifdef _SC_BUILD_
static int bcmxtmrt_set_mac_addr(struct net_device *dev, void *p);
#endif
...
#ifdef _SC_BUILD_
      .ndo_set_mac_address=bcmxtmrt_set_mac_addr,
#endif
...
#ifdef _SC_BUILD_
/***************************************************************************
 * Function Name: bcmxtmrt_set_mac_addr
 * Description  : Called to change device's mac addr. 
...
static int bcmxtmrt_set_mac_addr(struct net_device *dev, void *p)
{
    ...
    if(netif_running(dev))
        return -EBUSY;
    memcpy(dev->dev_addr, addr->sa_data, NVRAM_MAC_ADDRESS_LEN);
    printk("<%s %d>new mac %02x:%02x:%02x:%02x:%02x:%02x\n", __func__, __LINE__, dev->dev_addr[0], dev->dev_addr[1], dev->dev_addr[2], dev->dev_addr[3], dev->dev_addr[4], dev->dev_addr[5]);
    return 0;
}
#endif
...
#ifdef _SC_BUILD_
{
		struct mac_entry_t *pos;
		pos = kmalloc(sizeof(struct mac_entry_t), GFP_KERNEL);
		if (pos) 
		{
			pos->ifindex = dev->ifindex;
			memcpy(pos->defMac, dev->dev_addr, NVRAM_MAC_ADDRESS_LEN);
			list_add(&pos->list, &macHead);	
		}
}
#endif
      }
...
#ifdef _SC_BUILD_
               /* gordon add for clear mib stats after device is up*/
                 bcmxtmrt_clrStats(pDevCtx->pDev);
#endif
            }
            else
            {
...
#ifdef _SC_BUILD_
{
            struct mac_entry_t *pos=NULL, *next=NULL;
            list_for_each_entry_safe(pos, next, &macHead, list)
            {
                if (pos->ifindex == pDevCtx->pDev->ifindex)
                {
                   // kerSysReleaseMacAddress( pos->defMac );
                    list_del(&pos->list);
                    kfree(pos); 
                    break;
                }
