   ...
   {
      {
         ...
         BP_VD_ZARLINK_9662,
         BP_SPI_SS_NOT_REQUIRED,  /* ZSI SPI CS handled internally. It is mapped using the zsiMapList */
#if defined(_BCM963268_) || defined(CONFIG_BCM963268)   /*Sercomm Jacky,for F1*/   
...
static bp_elem_t g_voice_bcm963381ref1[] = {                 
   ...
   {bp_ulInterfaceEnable,       .u.ul = BP_PINMUX_FNTYPE_HS_SPI},
   {bp_ulInterfaceEnable,       .u.ul = BP_PINMUX_FNTYPE_PCM},
   {bp_usFxsFxoRst1,            .u.us = BP_NOT_CONNECTED},
...
#if 1   /*Sercomm Jacky*/
            bpmemcpy(&currentVoiceParms.voiceDevice[nDeviceCount].channel[0], &((*ppDc)->voiceDevice[i].channel[0]), sizeof(BP_VOICE_CHANNEL)*BP_MAX_CHANNELS_PER_DEVICE);
#else
...
#if defined(_BCM963268_) || defined(CONFIG_BCM963268)   /*Sercomm ,Jacky for F1*/
       currentVoiceParms.numDectLines = 4;
#endif
         /* Copy over params */
